...
char** get_all( hashtable_Element *hashtable ){

	char** movielist = malloc(2*5000*(hashtable->size));

	for( int i = 0; i < 2*(hashtable->size); i++ ){
		movielist[i] = calloc(5000, sizeof(int));
	}
	...
	for( int i = 0; i < hashtable->size; i++ ){
		...
		if(hashtable->table[i] != NULL){
			...
			if(hashtable->table[i]->key != NULL)
				strcpy(movielist[index], hashtable->table[i]->key);

			index++;
			if(hashtable->table[i]->value != NULL)
				strcpy(movielist[index], hashtable->table[i]->value);
			index++;

			entry_t *element = hashtable->table[i];

			while(element->next != NULL){

				strcpy(movielist[index], element->key);
				index++;
				strcpy(movielist[index], element->value);
				index++;
...
void delete_all( hashtable_Element *hashtable ){
